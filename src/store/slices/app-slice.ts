...
import { StakingContract, MoonTokenContract, ApoloTokenContract } from "../../abi";
...
export const loadAppDetails = createAsyncThunk(
    ...
    async ({ networkID, provider }: ILoadAppDetails) => {
        ...
        const moonContract = new ethers.Contract(addresses.MOON_ADDRESS, MoonTokenContract, provider);
        const apoloContract = new ethers.Contract(addresses.APOLO_ADDRESS, ApoloTokenContract, provider);

        const marketPrice = ((await getMarketPrice(networkID, provider)) / Math.pow(10, 9)) * mimPrice;

        const totalSupply = (await apoloContract.totalSupply()) / Math.pow(10, 9);
        const circSupply = (await moonContract.circulatingSupply()) / Math.pow(10, 9);
            console.log("sasasassa",circSupply)
        ...
        const circ = await moonContract.circulatingSupply();
