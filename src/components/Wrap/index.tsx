...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const wmoonBalance = useSelector<IReduxState, string>(state => {
        return state.account.balances && state.account.balances.wmoon;
    });
    ...
    const moonAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.moon;
    });
    ...
    const setMax = () => {
        if (isWrap) {
            setValue(moonBalance);
        } else {
            setValue(wmoonBalance);
        }
    };
    ...
    const hasAllowance = useCallback(() => moonAllowance > 0, [moonAllowance]);

    const trimmedMoonBalance = trim(Number(moonBalance), 6);
    const trimmedWmoonBalance = trim(Number(wmoonBalance), 6);

    const getBalance = () => (isWrap ? `${trimmedMoonBalance} MOON` : `${trimmedWmoonBalance} wMOON`);
            ...
            <Paper className="ohm-card ohm-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "MOON" : "wMOON"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wMOON" : "MOON"}`}
                        </p>
                    </div>
                </div>
